...
public class SqlUtil
  {
    public const string strOLBTreeRegistryVersion = @"Software\ESRI\OLBTreeDotNet_DailyBuild\Enterprise";
    /// <summary>
    /// web sql connection string
    /// </summary>
    public static string SqlConnectionStr = GetHostConnectionString();
    private static string _sqlConnectionStr = string.Empty;

    private SqlConnection? sqlconnection = null;
    private string sqlerror = "";

    private static string GetHostConnectionString()
    {
      if (string.IsNullOrEmpty(_sqlConnectionStr))
      {
        RegTools Regtool = new();
        _sqlConnectionStr = Regtool.QueryValue(strOLBTreeRegistryVersion, "DbConnectionStr");
        // if we don't have the connection string from the registry, let's try to make one
        // Data Source=SDKAODOC;database=EnterpriseSDK;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=True;ApplicationIntent=ReadWrite;MultiSubnetFailover=False
        if (string.IsNullOrEmpty(_sqlConnectionStr))
        {
          _sqlConnectionStr = $"Data Source=sdkaodoc;database=EnterpriseSDK;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=True;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
          // throw new Exception($"Error: registry does not defines: {strOLBTreeRegistryVersion} {_sqlConnectionStr} use OLBTreeDotNet_DailyBuild-10.x.reg");
        }
      }
    ...
    public SqlUtil()
    {
      ...
      {
        sqlerror = $"Unable to open database {SqlConnectionStr}: [{ex}]";
      }
    }
