...
namespace OlbLib
{
  public static class TlbWrite
  {
    ...
    public static string WriteXmlDocument(TlbLibCollection tlbLibCollection, string outputRoot, string namespaceName)
    {
      ...
      File.Delete(@"C:\Temp\TlbWrite.xml");
      // add members for interfaces
      File.AppendAllText(@"C:\Temp\TlbWrite.xml", "Interfaces");
      foreach (var interfaceInfo in currentLib.InterfaceInfos) {
        WriteInterfaceXMLDocument(members, interfaceInfo);
        File.AppendAllText(@"C:\Temp\TlbWrite.xml", members.ToString());
      }
      // add members for classes
      File.AppendAllText(@"C:\Temp\TlbWrite.xml", "Coclasses");
      ...
      File.AppendAllText(@"C:\Temp\TlbWrite.xml", "Constants");
      foreach (var constantInfo in currentLib.ConstantInfos)
      {
        WriteEnumXMLDocument(members, constantInfo);
        // check if members.ToString() has invalid characters
        foreach (char c in members.ToString())
        {
          if (char.IsControl(c) && c != '\r' && c != '\n') {
          System.Diagnostics.Trace.WriteLine($"Invalid character in XML: {c} (ASCII {(int)c})");
          }
        }
        File.AppendAllText(@"C:\Temp\TlbWrite.xml", members.ToString());
      }
    ...
    private static void WriteInterfaceXMLDocument(XElement members, TlbInterfaceInfo interfaceInfo)
    {
      ...
      var interfaceName = $"T:{ns}.{interfaceInfo.Name}";
      var interfaceMember = new XElement("member",
          new XAttribute("name", interfaceName),
      ...
      {
        var memberName = $"M:{ns}.{interfaceInfo.Name}.{member.Name}";
        var memberNode = new XElement("member",
                new XAttribute("name", MemberHelper.ToXmlDocMemberName(member.ManagedMemberInfo)),
    ...
    private static void WriteEnumXMLDocument(XElement members, TlbConstantInfo constantInfo)
    {
      ...
      foreach (var variable in constantInfo.Variables)
      {
        ...
        var enumValueMember = new XElement("member", 
                  ...
                  new XElement("summary", $@"{variable.Value}: {variable.HelpString ?? string.Empty}")
                );
        System.Diagnostics.Trace.WriteLine($"F:{enumMemberName}.{variable.Name}");
        System.Diagnostics.Trace.WriteLine($@"{variable.Value}: {variable.HelpString}");
        foreach (char c in members.ToString())
        {
          if (char.IsControl(c) && c != '\r' && c != '\n')
          {
