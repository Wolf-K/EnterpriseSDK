...
namespace MyReflection
{
  public class MyNamespace : MyTypeBase
  {
    ...
    /// <summary>
    /// Write the .xml api documentation file for the namespace, which includes the list of types in that namespace and their descriptions. The .mdx file is created in a subfolder named after the namespace under the outputRoot folder.
    /// </summary>
    /// <param name="outputRoot"></param>
    /// <param name="dllFileName"></param>
    public override void WriteXML(string outputRoot, string dllFileName)
    {
      ...
      foreach (var myInterface in interfaces)
      {
        members.Add(myInterface.CreateXML());
      }

      // write all classes
      foreach (var myClass in classes)
      {
        members.Add(myClass.CreateXML());
      }

      // write all constants (enums)
      foreach (var myConstant in constants)
      {
        members.Add(myConstant.CreateXML());
      }
      var xmlDoc = new XDocument(
        new XElement("doc",
          new XElement("assembly",
            new XElement("name", dllFileName)
          ),
          members)
      );
    ...
    {
      Console.WriteLine($@"CreateXML: {FullName} Namespace");
      return [];
    }
  }
}
